...
&& sleep 3 \
&& cd ${MYSQL_SCHEMA_DIR} \
&& mysql -u ${MYSQL_USER} < 1_uic-db-schema.sql \
&& mysql -u ${MYSQL_USER} < 2_portal-db-schema.sql \
&& mysql -u ${MYSQL_USER} < 3_dashboard-db-schema.sql \
&& mysql -u ${MYSQL_USER} < 4_graph-db-schema.sql \
&& mysql -u ${MYSQL_USER} < 5_alarms-db-schema.sql \
...
./open-falcon check
./open-falcon monitor hbs # keep container run
